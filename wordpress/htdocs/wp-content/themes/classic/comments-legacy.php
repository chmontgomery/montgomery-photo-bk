		...
		<li class="comment" id="comment-<?php comment_ID() ?>">	
         ...
         ?>
            ...
            <div class="content">
                ...
                <em><?php _e('Your comment is awaiting moderation.',photographythemes); ?></em>
                <?php endif; ?>
                <br />
...
<h2 class="commh2"><?php _e('Leave a Reply',photographythemes); ?></h2>

<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
<p class="alert"><?php _e('You must be',photographythemes); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>"><?php _e('logged in',photographythemes); ?></a> <?php _e('to post a comment.',photographythemes); ?></p>
<?php else : ?>
...
<p><?php _e('Logged in as',photographythemes); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account"><?php _e('Logout &raquo;',photographythemes); ?></a></p>

<?php else : ?>

<p><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" />
<label for="author"><small><?php _e('Name',photographythemes); ?> <?php if ($req) echo  __('(required)',photographythemes); ?></small></label></p>

<p><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" />
<label for="email"><small><?php _e('Mail (will not be published)',photographythemes); ?> <?php if ($req) echo  __('(required)',photographythemes); ?></small></label></p>
...
<p><input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Comment',photographythemes); ?>" />
