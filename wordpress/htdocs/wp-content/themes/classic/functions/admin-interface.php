...
function photographythemes_machine($options,$page) {
    ...
    foreach ($options as $value) {
        ...
        case 'text':
            ...
            if ( get_option( $value['id'] ) != "") { $val = get_option($value['id']); }
        ...
        case 'textarea':
            ...
            if( get_option($value['id']) != "") { $ta_value = stripslashes(get_option($value['id'])); }
...
function photographythemes_uploader_function($id,$std){
    ...
    $uploader .= '<img src="'.$upload.'" width="290" height="200" alt="" />';
