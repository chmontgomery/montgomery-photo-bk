...
(function ($) {
    ...
    $.fn.popeye = function (options) {
		...
		var slideshow; // SHIBA GALLERY - need to make this local to function
        ...
        return this.each(function(){
            ...
            cssSelf = {
                ...
                width:          'auto',
                height:         'auto',
				'max-width': (opts.responsive) ? '100%' : 'none', // Shiba Gallery
                margin:         0,
                top:            0,
...
//                'width':          (opts.caption == 'hover' || false) ? ppyOuter.outerWidth() : $self.outerWidth(),
				...
				'max-width': (opts.responsive) ? '100%' : 'none', 
				'margin':		'auto',
...
//                cap[i]   = extcap.length > 0 ? extcap.html() : img[i].alt;
                cap[i]   = extcap.length > 0 ? extcap.children() /*.find('a')*/ : img[i].alt;
            }
            ...
            //------------------------------------------------------------------
                ...
                function showThumb(i, transition) {
                    ...
                    if(enlarged) {
						...
						// Shiba Gallery - turn on max-width
						if (opts.responsive) {
							$self.css(cssSelfRestore);
							ppyStage.css(cssStageRestore);
                            adjustHeight(); 
						}
                       }
...
//					console.log("Adjust Height");
					...
					if (opts.aspect) {
						if (ppyOuter.width() == origWidth)
							return;
						origWidth = ppyOuter.width();
						
						var h = Math.floor(opts.aspect * ppyOuter.width());
//						console.log(h);
						$self.height(h);
						ppyStage.height(h);
					}
                ...
                function showImage(i) {
					...
					// Shiba Gallery - turn off max-width
					$self.css('max-width', 'none');
					ppyStage.css('max-width', 'none');
...
//                        ppyText.html(caption);
						// Shiba Gallery
//						console.log(caption);
//						console.log($._data( $(caption)[0], "events" )); // Show events
						ppyText.children().detach();
						ppyText.append(caption);
                    }
                }
                ...
                function init() {
					...
					} else { // not active go next
						ppyStage.click(function(){ next(); });
					}
					
					if (opts.aspect) {
						shibaResizeFunctions.push(adjustHeight.bind(this));
					}
